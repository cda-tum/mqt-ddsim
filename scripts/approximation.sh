...
#
for file in ./circuits/inst_4x5*.txt; do
  base="$(basename -s .txt "$file")"
  sem -j 3 benchmark simmem0925-"${base}" --ps --approx_when memory --step_fidelity 0.925 --simulate_file "${file}"
  sem -j 3 benchmark simmem0950-"${base}" --ps --approx_when memory --step_fidelity 0.950 --simulate_file "${file}"
  sem -j 3 benchmark simmem0975-"${base}" --ps --approx_when memory --step_fidelity 0.975 --simulate_file "${file}"
  sem -j 3 benchmark simmem0990-"${base}" --ps --approx_when memory --step_fidelity 0.990 --simulate_file "${file}"
  sem -j 3 benchmark simmem1000-"${base}" --ps --approx_when memory --step_fidelity 1.000 --simulate_file "${file}"
done
