...
public:
  /**
   * @brief creates a Heuristic based initialLayout for the QuantumComputation and applies it. 
            The current implementation is based on pattern found in the controlled gates
  ...
  */
   ...
   * @brief creates a Heuristic based initialLayout for the QuantumComputation.
            This implementation is based on pattern found in the controlled gates
   * @param QuantumComputation
  ...
  */
  ...
  // Helper functions for createGateBasedPermutation
    ...
    * @return a pair of maps: 
            1. map string of c_x, x_c, c_l, x_l, c_r, x_r ladder step to control and target bit to the index of the gate
            2. map: string of c_x, x_c, c_l, x_l, c_r, x_r ladder to vector of max index of gate
          for the indicees:      
    ...
    */
    static std::pair<std::map<std::string, std::map<std::pair<Qubit, Qubit>, int>>,
  ...
  /**
   * @brief creates a Heuristic based initialLayout for the QuantumComputation.
            This implementation is based on which qubits are controlled by which qubits
   * @param QuantumComputation
   * @return the qc::Permutation
   */
  ...
  // Helper function for createControlBasedPermutation
    ...
    /**
    * @brief recursively adjusts the weights of the qubits based on the control and target qubits
    * @param map of qubit to weight, current set of target qubits, current control qubit, overall map of control qubit to target qubits, count of recoursive calls
  ...
  * @brief creates a vector of all possible permutations of the initialLayout
            used for first implementation of testing all Permutations and drawing conclusions
  * @param QuantumComputation
  */
