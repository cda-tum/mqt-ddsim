...
def execute_circuit(qc: QuantumCircuit, backend, shots: int, mode: Union[str, ddsim.PathSimulatorMode] = 'sequential', **options):
    ...
    result = execute_verification(qc, qc, backend=backend, gatecost=[], shots=shots, mode=mode, optimization_level=0,
                                  **options).result()
...
def execute_verification(qc: QuantumCircuit, qcog: QuantumCircuit, gatecost, backend, shots: int,
    ...
    if mode == 'alternating':
        ...
        alt_start = qcog.size()
        sim = ddsim.PathCircuitSimulator(circ=qc, mode=ddsim.PathSimulatorMode(mode), alternating_start=alt_start, gate_cost=gatecost)
    elif mode == 'gatecost':
        print('number of gates in the original circuit and starting point', qcog.size())
        alt_start = qcog.size()
        ...
        sim = ddsim.PathCircuitSimulator(circ=qc, mode=ddsim.PathSimulatorMode(mode), gate_cost=gatecost)  # ccomp changed to qc
    if mode == 'cotengra':
...
if __name__ == '__main__':
    ...
    with open(
        ...
        for line in f.readlines():
            line = line.split(" ")
            lut_gatecost[line[0] + line[1]] = str(line[2][:-1])
    ...
    for n in [10, 11, 12, 13, 14]:
        ...
        #print(result)
        execute_verification_all(qc=qccomp, qcog=qc, backend=backend, gatecosts=gatecosts, shots=shots, include_cotengra=True,
    ...
    for n in [14, 15, 16, 17, 18]:
        ...
        #result = sim.simulate(1000)
        #print(result)
        execute_verification_all(qc=qccomp, qcog=qc, gatecosts=gatecosts, backend=backend, shots=shots, include_cotengra=True,
                                 max_time=max_time, max_repeats=max_repeats, plot_ring=plot_ring)
    ...
    for n in [46, 48, 50, 52, 54, 56]:
        ...
        #result = sim.simulate(1000)
        #print(result)
        execute_verification_all(qccomp, qc, gatecosts, backend, shots, include_cotengra=True, max_time=max_time, max_repeats=max_repeats, plot_ring=plot_ring)
    ...
    for n in [10, 11, 12, 13, 14, 15, 16, 17]:
        ...
        for i in range(qc.size()):
            if qc[i][0].name == 'u3':
                index = 'u' + str(len(qc[i][1]) - 1)
            elif qc[i][0].name == 'cx' or qc[i][0].name == 'mcx' or qc[i][0].name == 'mcx_gray' or qc[i][0].name == 'ccx' or qc[i][0].name == 'mcx_vchain':
                index = 'x' + str(len(qc[i][1]) - 1)
            elif qc[i][0].name == 'mcphase' or qc[i][0].name == 'cp':
                index = 'p' + str(len(qc[i][1])-1)
            else:
