...
TEST_F(DDVis, UnitarySimulator) {
  ...
  const auto dot =
      sim.exportDDtoGraphvizString(true, false, false, false, true);
  EXPECT_FALSE(dot.empty());
  std::cout << dot << "\n";
...
TEST_F(DDVis, HSFSimulatorDD) {
  HybridSchrodingerFeynmanSimulator sim(
      std::move(qc), HybridSchrodingerFeynmanSimulator<>::Mode::DD);
  sim.simulate(numShots);
...
TEST_F(DDVis, HSFSimulatorAmplitude) {
  HybridSchrodingerFeynmanSimulator sim(
      std::move(qc), HybridSchrodingerFeynmanSimulator<>::Mode::Amplitude);
  sim.simulate(numShots);
