...
TEST(StochNoiseSimTest, SimulateAdder4TrackFidelityWithNoise) {
    ...
    EXPECT_GE(m.find("1000")->second, 0.14);     EXPECT_LE(m.find("1000")->second, 0.16);
    EXPECT_GE(m.find("1001")->second, 0.54);     EXPECT_LE(m.find("1001")->second, 0.56);
...
TEST(StochNoiseSimTest, SimulateClassicControlledOpWithError) {
    ...
    EXPECT_GE(m.find("00")->second, 0.03); EXPECT_LE(m.find("00")->second, 0.06);
    EXPECT_GE(m.find("01")->second, 0.46); EXPECT_LE(m.find("01")->second, 0.48);
    EXPECT_GE(m.find("10")->second, 0.03); EXPECT_LE(m.find("10")->second, 0.05);
    EXPECT_GE(m.find("11")->second, 0.42); EXPECT_LE(m.find("11")->second, 0.44);
}
