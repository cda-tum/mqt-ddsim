...
int main(int argc, char** argv) { // NOLINT(bugprone-exception-escape)
  cxxopts::Options options(
      "FRQI",
      "with MQT DDSIM by https://www.cda.cit.tum.de/ -- Allowed options");
  // clang-format off
    options.add_options()
  ...
  for (it = dest.begin<double>(), end = dest.end<double>(); it != end; ++it) {
    if (*it != 0) {
      ...
      for (int i = 0; i < 10; i++) {
        controls.insert({static_cast<qc::Qubit>(i + 1),
  ...
  const std::map<std::string, std::size_t> mCounter =
      ddsim.simulate(numOfShots);
  ...
  for (const auto& [bitstring, count] : mCounter) {
    const std::size_t fixedIndex =
        std::stoul(bitstring.substr(0, 10), nullptr, 2);
    const double tmp =
        std::sqrt(static_cast<double>(count) / numOfShots) * 32. * 255.;
    if (vm.count("verbose") > 0) {
      std::clog << bitstring << " (" << fixedIndex << ") : " << count << " ("
                << tmp << ")\n";
    }
