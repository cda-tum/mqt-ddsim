...
bool isPrime(std::uint64_t number) {
  const auto upperLimit =
      static_cast<std::uint64_t>(std::floor(std::sqrt(number)));
...
void outputCoprimes(const std::uint64_t compositeNumber,
...
void outputPrimes(const std::uint64_t compositeNumber,
                  const std::uint64_t length) {
  std::uint64_t outputLength = 0;
  std::uint64_t currentNumber = 2;
...
int main(int argc, char** argv) { // NOLINT(bugprone-exception-escape)
  ...
  if (vm["strategy"].as<std::string>() == "coprimes") {
    outputCoprimes(vm["composite_number"].as<unsigned int>(),
                   vm["length"].as<unsigned int>());
  } else if (vm["strategy"].as<std::string>() == "primes") {
    outputPrimes(vm["composite_number"].as<unsigned int>(),
                 vm["length"].as<unsigned int>());
  } else {
