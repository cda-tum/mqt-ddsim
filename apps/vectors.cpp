...
int main(int argc, char** argv) {
  ...
  if (argc != 3) {
    std::cout << "Usage: " << argv[0] << " file1 file2\n"
              << "Calculates the fidelity between two vectors given as file "
                 "(one complex number per line).\n"
              << "Return code is 0 if fidelity is greater than 0.999 and 1 "
                 "otherwise. 2 for errors.\n";
    return 2;
  }
  ...
  while (!v1Stream.eof() && !v2Stream.eof()) {
    ...
    // istreams cannot read doubles in hexfloat format, so we have to use
    // strtold
