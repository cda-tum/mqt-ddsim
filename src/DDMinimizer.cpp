...
qc::Permutation DDMinimizer::createGateBasedPermutation(qc::QuantumComputation& qc){
    ...
    if((c_x->second[0] != -1) && (x_c->second[0] == -1 || c_x->second[0] < x_c->second[0])){

        if((prio_c_r == 0 && DDMinimizer::isFull(c_r->second)) || (prio_x_l == 0 && DDMinimizer::isFull(x_l->second))){  
        }
        ...
        else if(prio_c_r > 0 || prio_x_l > 0 || (prio_c_r == 0 && prio_x_l == 0 )) {
            layout = DDMinimizer::reverseLayout(layout);
        }
    }
    else if((x_c->second[0] != -1) && (c_x->second[0] == -1 || c_x->second[0] > x_c->second[0])){
    
        if((prio_c_l == 0 && DDMinimizer::isFull(c_l->second)) || (prio_x_r == 0 && DDMinimizer::isFull(x_r->second))){
            layout = DDMinimizer::reverseLayout(layout);
        }
        ...
        else if(prio_c_l > 0 || prio_x_r > 0 || (prio_c_l == 0 && prio_x_r == 0 )) {
            //do nothing cause identity is what we need anyway
        }
    }
    else if( (DDMinimizer::isFull(x_r->second) || DDMinimizer::isFull(c_l->second))){
        layout = DDMinimizer::reverseLayout(layout);
    }
    else {
...
std::vector<Qubit> DDMinimizer::rotateRight(std::vector<Qubit> layout, int stairs) {
    std::vector<Qubit>::size_type size = layout.size();
    std::vector<Qubit> rotated_layout(size);
    for (int r = 0; r < stairs; ++r) {
        if (!layout.empty()) {
            rotated_layout[size - static_cast<std::vector<Qubit>::size_type>(r + 1)] = layout[static_cast<std::vector<Qubit>::size_type>(r)];
        }
    }
    std::vector<Qubit>::size_type left = size - static_cast<std::vector<Qubit>::size_type>(stairs);
    for (std::vector<Qubit>::size_type r = 0; r < left; ++r) {
        if (!layout.empty()) {
            rotated_layout[r] = layout[r+static_cast<std::vector<Qubit>::size_type>(stairs)];
        }
    }
    return rotated_layout;
}

std::vector<Qubit> DDMinimizer::rotateLeft(std::vector<Qubit> layout, int stairs) {
      std::vector<Qubit>::size_type size = layout.size();
    std::vector<Qubit> rotated_layout(size);
    for (int r = 0; r < stairs; ++r) {
        if (!layout.empty()) {
            rotated_layout[static_cast<std::vector<Qubit>::size_type>(r)] = layout[size - static_cast<std::vector<Qubit>::size_type>(r + 1)];
        }
    }
    std::vector<Qubit>::size_type left = size - static_cast<std::vector<Qubit>::size_type>(stairs);
    for (std::vector<Qubit>::size_type r = 0; r < left; ++r) {
        if (!layout.empty()) {
            rotated_layout[r + static_cast<std::vector<Qubit>::size_type>(stairs)] = layout[r];
        }
    }
    return rotated_layout;
}
...
DDMinimizer::createControlBasedPermutation(qc::QuantumComputation& qc) {
  ...
  for (const auto& pair : controlToTargets) {
    if (encounteredTargets.find(pair.first) != encounteredTargets.end()) {
          qubitWeights = DDMinimizer::adjustWeights(qubitWeights, encounteredTargets, pair.first, controlToTargets, 1);
      }
...
std::map<Qubit, int> DDMinimizer::adjustWeights(std::map<Qubit, int> qubitWeights, std::set<Qubit> targets, Qubit ctrl, std::map<Qubit, std::set<Qubit>> controlToTargets, int count) {
    ...
    for (const auto& controlPair : controlToTargets) {
        if (controlPair.second.find(ctrl) != controlPair.second.end()) {
            ...
            qubitWeights = DDMinimizer::adjustWeights(qubitWeights, targets, control, controlToTargets, count + 1);
        }
    }
    return qubitWeights;
}
