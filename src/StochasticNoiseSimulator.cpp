...
std::map<std::string, std::size_t> StochasticNoiseSimulator::simulate(const size_t nshots) {
...
void StochasticNoiseSimulator::runStochSimulationForId(std::size_t stochRun, qc::Qubit nQubits, std::map<std::string, size_t>& classicalMeasurementsMap, std::uint64_t localSeed) {
    ...
    const auto          approxMod    = static_cast<unsigned>(std::ceil(static_cast<double>(qc->getNops()) / (static_cast<double>(approximationInfo.stepNumber + 1))));
    ...
    for (std::size_t currentRun = 0U; currentRun < numberOfRuns; currentRun++) {
        ...
        for (auto& op: *qc) {
            ...
            ++opCount;
            if (auto* nuOp = dynamic_cast<qc::NonUnitaryOperation*>(op.get()); nuOp != nullptr) {
            ...
            stochasticNoiseFunctionality.applyNoiseOperation(op->getUsedQubits(), operation, localRootEdge, generator);
            if (approximationInfo.stepFidelity < 1. && (opCount % approxMod == 0U)) {
                approximateByFidelity(localDD, localRootEdge, approximationInfo.stepFidelity, false, true);
                ++approximationRuns;
            }
            localDD->garbageCollect();
        }
